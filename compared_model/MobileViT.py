...
from compared_model.transformer import TransformerEncoder
from compared_model.model_config import get_config
...
class MobileViT(nn.Module):
    ...
    @staticmethod
    def _make_mit_layer(input_channel: int, cfg: Dict) -> [nn.Sequential, int]: # type: ignore
...
class woclassifier_MobileViT(nn.Module):
    """
    This class implements the `MobileViT architecture <https://arxiv.org/abs/2110.02178?context=cs.LG>`_
    """
    def __init__(self, model_cfg: Dict, num_classes: int = 1000):
...
def woclassifier_mobile_vit_xx_small(num_classes: int = 1000):
    # pretrain weight link
    # https://docs-assets.developer.apple.com/ml-research/models/cvnets/classification/mobilevit_xxs.pt
    config = get_config("xx_small")
    m = woclassifier_MobileViT(config, num_classes=num_classes)
    return m
