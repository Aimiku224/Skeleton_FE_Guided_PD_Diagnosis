...
class wofc_RegHead(nn.Module):
    def __init__(self,
                 in_unit: int = 368,
                 out_unit: int = 1000,
                 output_size: tuple = (1, 1),
                 drop_ratio: float = 0.25):
        super(wofc_RegHead, self).__init__()
        self.pool = nn.AdaptiveAvgPool2d(output_size)
...
class wofc_RegNet(nn.Module):
    ...
    def __init__(self,
                 ...
                 num_classes: int = 1000,
                 zero_init_last_bn: bool = True):
        super(wofc_RegNet, self).__init__()
        ...
        # RegHead
        self.head = wofc_RegHead(in_unit=input_channels, out_unit=num_classes)
...
def create_wofc_regnet(model_name="RegNetX_200MF", num_classes=1000):
    model_name = model_name.lower().replace("-", "_")
    if model_name not in model_cfgs.keys():
        print("support model name: \n{}".format("\n".join(model_cfgs.keys())))
        raise KeyError("not support model name: {}".format(model_name))

    model = wofc_RegNet(cfg=model_cfgs[model_name], num_classes=num_classes)
    return model
