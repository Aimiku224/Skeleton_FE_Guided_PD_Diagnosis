...
from compared_model.ConvNeXt import convnext_tiny as create_model
from compared_model.GoogleNet import GoogLeNet
from compared_model.DenseNet import densenet121, load_state_dict
from compared_model.EfficientNetV2 import efficientnetv2_s
from compared_model.RegNet import create_regnet
from compared_model.ResNet import resnext50_32x4d
from compared_model.SwinTransformer import swin_tiny_patch4_window7_224
from compared_model.MobileViT import mobile_vit_xx_small
from compared_model.MobileNetV3 import mobilenet_v3_small
from compared_model.ShuffleNet import shufflenet_v2_x1_0
...
def run_train(args):
    ...
    if args.wandb:
        ...
        wadb = wandb.init(
            ...
            project="FER",
            #name = "none_pretrain_model",
            name=f"{premodel_filename}",
    ...
    # EfficientNetV2
        if args.model == 'EfficientNetV2':
            # 如果存在预训练权重则载入
            model = efficientnetv2_s(num_classes=6).to(device)
            if args.weights != "":
                if os.path.exists(args.weights):
                    weights_dict = torch.load(args.weights, map_location=device)
    ...
    # RegNet
        if args.model == 'RegNet':
            model = create_regnet(num_classes=6).to(device)
            if args.weights != "":
                if os.path.exists(args.weights):
                    weights_dict = torch.load(args.weights, map_location=device)
                    load_weights_dict = {k: v for k, v in weights_dict.items()
    ...
    # MobileViT
        if args.model == 'MobileViT':   
            model = mobile_vit_xx_small(num_classes=6).to(device)
            if args.weights != "":
                assert os.path.exists(args.weights), "weights file: '{}' not exist.".format(args.weights)
                weights_dict = torch.load(args.weights, map_location=device)
    ...
    # MobileNetV3
        if args.model == 'MobileNetV3':   
            model = mobilenet_v3_small(num_classes=6).to(device)
            if args.weights != "":
                if os.path.exists(args.weights):
                    weights_dict = torch.load(args.weights, map_location=device)
                    load_weights_dict = {k: v for k, v in weights_dict.items()
    ...
    # ShuffleNet
        if args.model == 'ShuffleNet': 
            model = shufflenet_v2_x1_0(num_classes=6).to(device)
            if args.weights != "":
                if os.path.exists(args.weights):
                    weights_dict = torch.load(args.weights, map_location=device)
                    load_weights_dict = {k: v for k, v in weights_dict.items()
        ...
        ckpt_path = os.path.join(args.ckpt_dir, 'ckpt_fer', args.ckptdir) # os.path.basename(args.weights)
        ...
        f_log.write("pretrained_model:" +'none'+ "\n")
        ...
        train_dataset = FerDataSet(args.data_path, args.label_path, args.train, mode='train',
        ...
        val_dataset = FerDataSet(args.data_path, args.label_path, args.validation, mode='eval',
        ...
        for i in range(1, args.epochs + 1):
            ...
            for batch_i, (imgs, targets) in enumerate(train_loader):
                ...
                # googlenet
                # outputs, aux_logits2, aux_logits1 = model(imgs.to(device))
                # loss0 = CE_criterion(outputs, targets.to(device))
                # loss1 = CE_criterion(aux_logits1, targets.to(device))
                # loss2 = CE_criterion(aux_logits2, targets.to(device))
                ...
                if args.wandb and i == args.epochs:
                    ...
                    for j in range(len(train_wrong_indices)):
                        ...
                        img = imgs[target]
                        image = wandb.Image(img, caption=f"epoch{i}-{batch_i}-{j}")
            ...
            with torch.no_grad():
                ...
                if val_acc > best_acc:
                    ...
                    ckpt_name = 'ckpt_fer_'+args.train[:-4]+'_'+str(int(val_acc*10000))+'.pth'
                    torch.save(model.state_dict(), os.path.join(ckpt_path,ckpt_name))
